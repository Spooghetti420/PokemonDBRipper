...
POKEDEX_PATTERN = re.compile(r'<a class="ent-name" href="\/pokedex\/([\s\S]+?)".+?View Pokedex for #(\d{3}) ([\s\S]+?)">.+?<\/a>.+?type\/\w+">(\w+)<\/a>(?:<br> *<a class="type-icon.+?"\/type\/\w+">(\w+))?[\s\S]+?<td class="cell-total">(?:\d+)<\/td>\s+<td class="cell-num">(\d+)<\/td>\s<td class="cell-num">(\d+)<\/td>\s<td class="cell-num">(\d+)<\/td>\s<td class="cell-num">(\d+)<\/td>\s<td class="cell-num">(\d+)<\/td>\s<td class="cell-num">(\d+)<\/td>\s')
def get_pokemon_list(page: str):
    all_pokemon = list(re.finditer(POKEDEX_PATTERN, page))[:590]
    out_data = []
    existing_names = set()
    for p in all_pokemon:
        ...
        try:
            ...
            group_list = [(p.group(i) if p.group(i) is not None else "none" ) for i in range(2, 12)]
            group_list[0] = str(int(group_list[0]))
            group_list[2] = group_list[2].lower()
            group_list[3] = group_list[3].lower()

            print(f"Extracted stats ({', '.join([f'{stat} = {p.group(i+6)}' for i, stat in enumerate(('HP', 'Attack', 'Defense', 'Sp. Atk', 'Sp. Def', 'Speed'))])})", end="\n\t")
            ...
            for (url, folder, suffix) in (
                ...
                with open(os.path.join(folder, f"{p.group(1)}{suffix}.png" ), mode="wb") as image:
                    image.write(img.content)
                    print(f"Downloaded {p.group(1)}{suffix}.png", end="\n\t")
            print()
